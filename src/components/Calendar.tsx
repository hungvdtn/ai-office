...
const getDayDetails = (date: Date) => {
  ...
  // 1. Khởi tạo dữ liệu cơ bản từ thư viện
  const solar = Solar.fromYmd(date.getFullYear(), date.getMonth() + 1, date.getDate());
  const lunar = solar.getLunar();
  
  // Lấy Tiết khí, Hỷ thần, Tài thần (Giữ nguyên logic)
  const tietKhi = JIE_QI_MAP[lunar.getJieQi() || lunar.getPrevJieQi().getName()] || "Đang cập nhật...";
  const hyThan = lunar.getPositionXiDesc();
  const taiThan = lunar.getPositionCaiDesc();

  // 2. Tính toán Nhị thập bát tú (Chu kỳ 28 ngày)
  ...
  // 3. Xử lý Logic lọc công việc (Trọng tâm: Không làm mất tính năng)
  const manualYiJi = getManualYiJi(TRUC_12_LOCAL.indexOf(evalData.trucName));
  let rawHop = [...manualYiJi.hop.split(', '), ...translateArray(lunar.getDayYi(), YI_JI_MAP)];
  let rawKy = [...manualYiJi.ky.split(', '), ...translateArray(lunar.getDayJi(), YI_JI_MAP)];

  // Áp dụng Category Mapping (Đã tiếp thu từ chuyên gia)
  const allStars = [...evalData.catTinh, ...evalData.hungTinh, ...evalData.folkTaboos];
  
  // Các bộ lọc chặn (Hard Skip) - Đảm bảo vẫn hoạt động
  if (allStars.some(s => ['Cô thần', 'Quả tú', 'Tam nương sát', 'Thọ tử', 'Sát chủ'].includes(s))) {
    rawHop = rawHop.filter(job => !['Cưới hỏi', 'Đính hôn'].some(k => job.includes(k)));
  }
  if (allStars.some(s => ['Thổ phủ', 'Kiếp sát', 'Sát chủ'].includes(s)) || evalData.trucName === 'Phá') {
    rawHop = rawHop.filter(job => !['Động thổ', 'Khởi công', 'Xây nhà'].some(k => job.includes(k)));
  }
  
  // 4. Xử lý logic hiển thị kết quả (Dùng hàm isSameMeaning để tránh trùng lặp)
  const hopText = filterAndFormatJobs(rawHop, rawKy);
  let kyText = formatKyText(rawKy, evalData);

  return {
    truc: evalData.trucName,
    sao: saoName,
    saoDesc: SAO_LUCK[saoName] === 'Cát' ? `Ngày có sao ${saoName} chiếu mệnh, Cát.` : `Ngày có sao ${saoName} chiếu mệnh, cần cẩn trọng.`,
    nguHanh: getFullNguHanhDay(dayInfo),
    hoangDaoType: getHoangDaoType(lunar, dayInfo),
    tietKhi,
    hyThan,
    taiThan,
    ...
    hop: hopText,
    ky: kyText,
    gioHoangDao: GIO_HOANG_DAO[CHI_CHU[dayInfo.chiIdx]],
    tuoiXung: getTuoiXung(dayInfo)
  };
};
